...
esp_err_t es8311_sample_frequency_config(es8311_handle_t dev, int mclk_frequency, int sample_frequency) {
    ...
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_CLK_MANAGER_REG03, (sel->fs_mode << 6) | sel->adc_osr), TAG, "");
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_CLK_MANAGER_REG04, sel->dac_osr), TAG, "");
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_CLK_MANAGER_REG05, ((sel->adc_div - 1) << 4) | (sel->dac_div - 1)), TAG, "");

    ESP_RETURN_ON_ERROR(es8311_read_reg(dev, ES8311_CLK_MANAGER_REG06, &regv), TAG, "");
    regv &= 0xE0;
    regv |= (sel->bclk_div < 19) ? (sel->bclk_div - 1) : sel->bclk_div;
    ...
    ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_CLK_MANAGER_REG08, sel->lrck_l), TAG, "");
...
static esp_err_t es8311_resolution_config(const es8311_resolution_t res, uint8_t *reg) {
    ...
    case ES8311_RESOLUTION_24: break;
...
esp_err_t es8311_init(es8311_handle_t dev, const es8311_clock_config_t *const clk_cfg, const es8311_resolution_t res_in, const es8311_resolution_t res_out) {
    ...
    if (!clk_cfg->mclk_from_mclk_pin) {
        ESP_RETURN_ON_FALSE(res_out == res_in, ESP_ERR_INVALID_ARG, TAG, "Resolution IN/OUT must match");
    }
